...
test('text and geometry can undo twice and redo twice in order', () => {
  ...
  const a = { kind: 'content', text: 'before', x: 10 },
    b = { kind: 'content', text: 'after', x: 10 },
    c = { kind: 'content', text: 'after', x: 42 };
  h.push(a);
  h.push(b);
  assert.deepEqual(h.undo(c), b);
  assert.deepEqual(h.undo(b), a);
  assert.deepEqual(h.redo(a), b);
  assert.deepEqual(h.redo(b), c);
...
test('new edit after undo discards the old forward route', () => {
  ...
  assert.equal(h.undo('c'), 'a');
  assert.equal(h.redo('a'), 'c');
});
test('viewport entries remain distinct from document content', () => {
  const h = new UndoHistory(),
    doc = { kind: 'content', workspace: { text: 'original' } },
    viewA = { kind: 'view', activeId: 'one', view: { x: 1, y: 2, scale: 1 } },
    viewB = { ...viewA, view: { x: 40, y: 50, scale: 0.8 } };
  h.push(doc);
  h.push(viewA);
  assert.equal(h.peekUndo().kind, 'view');
  assert.deepEqual(h.undo(viewB), viewA);
  assert.equal(h.peekRedo().kind, 'view');
  assert.equal(h.peekUndo().kind, 'content');
...
test('history is bounded, empty navigation is inert, and clearing removes both routes', () => {
  ...
  assert.equal(h.undo('unused'), null);
  h.push(1);
  h.push(2);
  h.push(3);
