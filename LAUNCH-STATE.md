...
## 操作の配置修正（2026-09-14、配布準備）

`codex/compact-board-controls` で矢印・ノート入口の配置変更と左下の兄弟追加ボタン削除を保存した。
本人が「公開サイトにも反映する」と返答したため、mainへ統合し同じ公開範囲で配布する。
配布完了までは下記v5を維持し、成否は配布結果を取得してから追記する。
